...
function makeIndexSpec(
  ...
  for (const spec of indexSpecs) {
    if (typeof spec === 'string') {
      key.set(spec, 1);
***REMOVED*** else if (Array.isArray(spec)) {
      key.set(spec[0], spec[1] ?? 1);
***REMOVED*** else if (spec instanceof Map) {
      for (const [property, value] of spec) {
        key.set(property, value);
  ***REMOVED***
***REMOVED*** else if (isObject(spec)) {
...
***REMOVED***
  ...
  override execute(server: Server, session: ClientSession | undefined, callback: Callback): void {
    ...
    const cursor = this.db.collection(this.collectionName).listIndexes({ session ***REMOVED***
