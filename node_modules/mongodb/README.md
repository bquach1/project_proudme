...
```js
const updateResult = await collection.updateOne({ a: 3 }, { $set: { b: 1 } ***REMOVED***
...
```js
const deleteResult = await collection.deleteMany({ a: 3 ***REMOVED***
...
```js
const indexName = await collection.createIndex({ a: 1 ***REMOVED***
...
try {
  await collection.insertOne({ _id: 1 ***REMOVED***
  await collection.insertOne({ _id: 1 ***REMOVED*** // duplicate key error
} catch (error) {
