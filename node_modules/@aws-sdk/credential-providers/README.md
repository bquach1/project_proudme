...
```javascript
//...
const client = new FooClient({ credentials: fromSSO({ profile: "my-sso-profile" ***REMOVED***
```
...
```javascript
//...
const client = new FooClient({ credentials: fromIni({ profile: "my-sso-profile" ***REMOVED***
```
...
const confusedDeputyPlugin = {
  applyToStack: (stack) => {
    stack.add(addConfusedDeputyMiddleware, { step: "finalizeRequest" ***REMOVED***
***REMOVED***,
};
