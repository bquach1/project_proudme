 ...
 *     const userDoc = new UserModel({ name: 'Foo' ***REMOVED***
 *     await userDoc.save();
 *
 *     // You also use a model to create queries:
 *     const userFromDb = await UserModel.findOne({ name: 'Foo' ***REMOVED***
 *
...
***REMOVED***
    this.constructor.exists(where, optionsWithCustomValues)
      .then(documentExists => {
        ...
        callback(null, { $where: where, matchedCount ***REMOVED***
  ***REMOVED***)
      .catch(callback);
    return;
***REMOVED***
...
***REMOVED***
    ...
    ***REMOVED*** else if (result.matchedCount != null) {
...
***REMOVED***
***REMOVED*** else if (/^\$p/.test(op)) {
    // potentially changing array positions
    increment.call(self);
***REMOVED*** else if (Array.isArray(val)) {
    // $set an array
    increment.call(self);
***REMOVED*** else if (/\.\d+\.|\.\d+$/.test(data.path)) {
...
***REMOVED***
  ...
  if (i === 0) {
    ...
    if (utils.isMongooseObject(value)) {
      value = value.toObject({ depopulate: 1, _isNested: true ***REMOVED***
***REMOVED*** else if (value.valueOf) {
      value = value.valueOf();
***REMOVED***
...
***REMOVED***
  ...
  while (i--) {
    ...
    if (utils.isMongooseObject(val)) {
      val = val.toObject({ depopulate: true, transform: false, _isNested: true ***REMOVED***
***REMOVED*** else if (Array.isArray(val)) {
      val = val.map(iter);
...
***REMOVED***
  ...
  if ((where && where._id && where._id.$__ || null) != null) {
    where._id = where._id.toObject({ transform: false, depopulate: true ***REMOVED***
***REMOVED***
  for (; d < len; ++d) {
    const data = dirty[d];
...
***REMOVED*** else if (utils.isMongooseArray(value) && value.$path() && value[arrayAtomicsSymbol]) {
      // arrays and other custom types (support plugins etc)
      handleAtomics(this, where, delta, data, value);
***REMOVED*** else if (value[MongooseBuffer.pathSymbol] && Buffer.isBuffer(value)) {
      // MongooseBuffer
 ...
 * #### Example:
 *
 *     await Character.deleteMany({***REMOVED***
 *     await Character.create({ name: 'Jean-Luc Picard' ***REMOVED***
 *
 *     await Character.exists({ name: /picard/i ***REMOVED*** // { _id: ... }
 *     await Character.exists({ name: /riker/i ***REMOVED*** // null
 *
 ...
 *     const BossSchema = new BaseSchema({ department: String ***REMOVED***
 *
 *     const Person = mongoose.model('Person', PersonSchema);
 *     const Boss = Person.discriminator('Boss', BossSchema);
 *     new Boss().__t; // "Boss". `__t` is the default `discriminatorKey`
 *
 *     const employeeSchema = new Schema({ boss: ObjectId ***REMOVED***
...
***REMOVED*** else if (ctx[modelSymbol] == null) {
    throw new MongooseError('`Model.' + fnName + '()` cannot run without a ' +
...
***REMOVED*** else if (typeof capped === 'object') {
...
***REMOVED***
  ...
  if (timeseries != null) {
    ...
    if (options.expireAfterSeconds != null) {
      // do nothing
***REMOVED*** else if (options.expires != null) {
      utils.expires(options);
***REMOVED*** else if (this.schema.options.expireAfterSeconds != null) {
      options.expireAfterSeconds = this.schema.options.expireAfterSeconds;
***REMOVED*** else if (this.schema.options.expires != null) {
 ...
 * #### Example:
 *
 *     const schema = new Schema({ name: { type: String, unique: true } ***REMOVED***
 *     const Customer = mongoose.model('Customer', schema);
 *     await Customer.collection.createIndex({ age: 1 ***REMOVED*** // Index is not in schema
...
***REMOVED***
  ...
  ***REMOVED***
      ...
      cb(null, { toDrop, toCreate ***REMOVED***
    ***REMOVED***
  ***REMOVED***
};
 ...
 * #### Example:
 *
 *     const res = await Character.remove({ name: 'Eddard Stark' ***REMOVED***
 ...
 * #### Example:
 *
 *     await Character.deleteOne({ name: 'Eddard Stark' ***REMOVED*** // returns {deletedCount: 1}
 *
 * #### Note:
 *
 ...
 * #### Example:
 *
 *     await Character.deleteMany({ name: /Stark/, age: { $gte: 18 } ***REMOVED*** // returns {deletedCount: x} where x is the number of documents deleted.
 *
 * #### Note:
 *
 ...
 *
 *     // find all documents
 *     await MyModel.find({***REMOVED***
 *
 *     // find all documents named john and at least 18
 *     await MyModel.find({ name: 'john', age: { $gte: 18 } }).exec();
 *
 *     // executes, passing results to callback
 *     MyModel.find({ name: 'john', age: { $gte: 18 }}, function (err, docs) {***REMOVED***
 *
...
Model.find = function find(conditions, projection, options, callback) {
  ...
  if (typeof conditions === 'function') {
    ...
    projection = null;
    options = null;
***REMOVED*** else if (typeof projection === 'function') {
    callback = projection;
    projection = null;
    options = null;
***REMOVED*** else if (typeof options === 'function') {
 ...
 *
 *     // using callback
 *     Adventure.findById(id, function (err, adventure) {***REMOVED***
 *
 ...
 *
 *     // using callback
 *     Adventure.findOne({ country: 'Croatia' }, function (err, adventure) {***REMOVED***
 *
 *     // select only the adventures name and length
...
***REMOVED*** else if (typeof projection === 'function') {
    ...
    projection = null;
    options = null;
***REMOVED*** else if (typeof conditions === 'function') {
    callback = conditions;
    conditions = {};
    projection = null;
    options = null;
 ...
 * #### Example:
 *
 *     const count = await Adventure.count({ type: 'jungle' ***REMOVED***
 *     console.log('there are %d jungle adventures', count);
 *
 * @deprecated
 * @param {Object} filter
...
Model.where = function where(path, val) {
  ...
  void val; // eslint
  const mq = new this.Query({}, {}, this, this.$__collection).find({***REMOVED***
 ...
 *
 *     Blog.$where('this.username.indexOf("val") !== -1').exec(function (err, docs) {***REMOVED***
 *
 ...
 *     // specify options.
 *     await Character.create([{ name: 'Jean-Luc Picard' }], { session ***REMOVED***
 *
        ...
        const callbackWrapper = (error, doc) => {
          if (error) {
            if (!firstError) {
              firstError = error;
        ***REMOVED***
            return callback(null, { error: error ***REMOVED***
      ***REMOVED***
          callback(null, { doc: doc ***REMOVED***
    ***REMOVED***;
 ...
 * #### Example:
 *
 *     const doc = await Person.create({ name: 'Ned Stark' ***REMOVED***
 ...
 *     let doc = await Person.findOne({ name: 'Ned Stark' }, null, { session ***REMOVED***
 *     await doc.remove();
 *     // `doc` will always be null, even if reading from a replica set
 *     // secondary. Without causal consistency, it is possible to
 *     // get a doc back from the below query if the query reads from a
 *     // secondary that is experiencing replication lag.
 *     doc = await Person.findOne({ name: 'Ned Stark' }, null, { session, readPreference: 'secondary' ***REMOVED***
 *
 ...
 *     Movies.insertMany(arr, function(error, docs) {***REMOVED***
 *
...
Model.bulkSave = async function(documents, options) {
  ...
  const writeOperations = this.buildBulkWriteOperations(documents, { skipValidation: true, timestamps: options.timestamps ***REMOVED***
 ...
 *
 *     const obj = Test.castObject({ num: '42' ***REMOVED***
 *     obj.num; // 42 as a number
 *
 *     Test.castObject({ num: 'not a number' ***REMOVED*** // Throws a ValidationError
 *
 ...
 *     const mongooseCandy = Candy.hydrate({ _id: '54108337212ffb6d459f854c', type: 'jelly bean' ***REMOVED***
 *
 * @param {Object} obj
 ...
 *
 *     const res = await MyModel.update({ name: 'Tobi' }, { ferret: true ***REMOVED***
 ...
 * #### Example:
 *
 *     const res = await Person.updateMany({ name: /Stark$/ }, { isDeleted: true ***REMOVED***
 ...
 * #### Example:
 *
 *     const res = await Person.updateOne({ name: 'Jean-Luc Picard' }, { ship: 'USS Enterprise' ***REMOVED***
 *     res.matchedCount; // Number of documents matched
 *     res.modifiedCount; // Number of documents modified
 *     res.acknowledged; // Boolean indicating everything went smoothly.
 *     res.upsertedId; // null or an id containing a document that had to be upserted.
 ...
 * #### Example:
 *
 *     const res = await Person.replaceOne({ _id: 24601 }, { name: 'Jean Valjean' ***REMOVED***
 *     res.matchedCount; // Number of documents matched
 *     res.modifiedCount; // Number of documents modified
 *     res.acknowledged; // Boolean indicating everything went smoothly.
 *     res.upsertedId; // null or an id containing a document that had to be upserted.
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    ***REMOVED***
        _checkDone();
  ***REMOVED***, context, { path: path ***REMOVED***
***REMOVED***
 ...
 *     ];
 *     await User.populate(users, { path: 'dog', select: 'name' ***REMOVED***
...
***REMOVED*** else if (options._localModel != null && model.base.options.strictPopulate != null) {
      populateOptions.strictPopulate = model.base.options.strictPopulate;
***REMOVED*** else if (model.base.options.strictPopulate != null) {
      populateOptions.strictPopulate = model.base.options.strictPopulate;
***REMOVED***
***REMOVED***
...
***REMOVED*** else if (mod.options.limit != null) {
...
***REMOVED*** else if (queryOptions.limit != null) {
  ...
  if (subPopulate) {
    ...
    if (mod.model.baseModelName != null) {
      if (Array.isArray(subPopulate)) {
        subPopulate.forEach(pop => { pop.strictPopulate = false; ***REMOVED***
  ***REMOVED*** else if (typeof subPopulate === 'string') {
