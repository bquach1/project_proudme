...
testFilter({ name: "sarah" ***REMOVED*** //true
testFilter({ name: "tim" ***REMOVED*** //false
```
...
const filter = createQueryTester({ $eq: 5 }, { operations: { $eq, $in } ***REMOVED***
```
...
```javascript
db.collection.find({ createdAt: { $gte: "2018-03-22T06:00:00Z" } ***REMOVED***
```
