...
```js
var q = mquery(collection).where({ _id: id }).setOptions({ overwrite: true ***REMOVED***
...
// GeoJSON
query.where('loc').near({ center: { type: 'Point', coordinates: [10, 10] }***REMOVED***
query.where('loc').near({ center: { type: 'Point', coordinates: [10, 10] }, maxDistance: 5, spherical: true ***REMOVED***
...
```js
// instead of writing:
mquery().find({age: {$gte: 21, $lte: 65}***REMOVED***
