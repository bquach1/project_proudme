...
export class CountDocumentsOperation extends AggregateOperation<number> {
  constructor(collection: Collection, query: Document, options: CountDocumentsOptions) {
    const pipeline = [];
    pipeline.push({ $match: query ***REMOVED***

    if (typeof options.skip === 'number') {
      pipeline.push({ $skip: options.skip ***REMOVED***
***REMOVED***

    if (typeof options.limit === 'number') {
      pipeline.push({ $limit: options.limit ***REMOVED***
***REMOVED***

    pipeline.push({ $group: { _id: 1, n: { $sum: 1 } } ***REMOVED***
