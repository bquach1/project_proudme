...
***REMOVED***, but this version of the Node.js Driver requires at least ${MIN_SUPPORTED_WIRE_VERSION} (MongoDB ${MIN_SUPPORTED_SERVER_VERSION})`;
  return new MongoCompatibilityError(message);
}
...
function performInitialHandshake(
  ...
  const callback: Callback<Document> = function (err, ret) {
    if (err && conn) {
      conn.destroy({ force: false ***REMOVED***
***REMOVED***
    _callback(err, ret);
***REMOVED***;
...
***REMOVED*** else if (typeof hostAddress.host === 'string') {
...
***REMOVED***
    socket = tlsSocket;
***REMOVED*** else if (existingSocket) {
