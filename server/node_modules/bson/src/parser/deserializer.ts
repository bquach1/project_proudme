...
***REMOVED*** else if (elementType === constants.BSON_DATA_OID) {
      const oid = Buffer.alloc(12);
      buffer.copy(oid, 0, index, index + 12);
      value = new ObjectId(oid);
      index = index + 12;
***REMOVED*** else if (elementType === constants.BSON_DATA_INT && promoteValues === false) {
      value = new Int32(
        buffer[index++] | (buffer[index++] << 8) | (buffer[index++] << 16) | (buffer[index++] << 24)
      );
***REMOVED*** else if (elementType === constants.BSON_DATA_INT) {
      value =
        buffer[index++] |
        (buffer[index++] << 8) |
        (buffer[index++] << 16) |
        (buffer[index++] << 24);
***REMOVED*** else if (elementType === constants.BSON_DATA_NUMBER && promoteValues === false) {
      value = new Double(dataview.getFloat64(index, true));
      index = index + 8;
***REMOVED*** else if (elementType === constants.BSON_DATA_NUMBER) {
      value = dataview.getFloat64(index, true);
      index = index + 8;
***REMOVED*** else if (elementType === constants.BSON_DATA_DATE) {
      const lowBits =
        buffer[index++] |
        (buffer[index++] << 8) |
        (buffer[index++] << 16) |
...
***REMOVED*** else if (elementType === constants.BSON_DATA_BOOLEAN) {
      if (buffer[index] !== 0 && buffer[index] !== 1)
        throw new BSONError('illegal boolean type value');
      value = buffer[index++] === 1;
***REMOVED*** else if (elementType === constants.BSON_DATA_OBJECT) {
      const _index = index;
      const objectSize =
        buffer[index] |
        (buffer[index + 1] << 8) |
...
***REMOVED*** else if (elementType === constants.BSON_DATA_ARRAY) {
      const _index = index;
      const objectSize =
        buffer[index] |
        (buffer[index + 1] << 8) |
...
***REMOVED*** else if (elementType === constants.BSON_DATA_UNDEFINED) {
      value = undefined;
***REMOVED*** else if (elementType === constants.BSON_DATA_NULL) {
      value = null;
***REMOVED*** else if (elementType === constants.BSON_DATA_LONG) {
  ...
  ***REMOVED***
        value = long;
  ***REMOVED***
***REMOVED*** else if (elementType === constants.BSON_DATA_DECIMAL128) {
  ...
  ***REMOVED***
        value = decimal128;
  ***REMOVED***
***REMOVED*** else if (elementType === constants.BSON_DATA_BINARY) {
      let binarySize =
        buffer[index++] |
        (buffer[index++] << 8) |
        (buffer[index++] << 16) |
  ...
  ***REMOVED***
    ...
    ***REMOVED***
        ...
        if (promoteBuffers && promoteValues) {
          value = _buffer;
    ***REMOVED*** else if (subType === constants.BSON_BINARY_SUBTYPE_UUID_NEW) {
...
***REMOVED*** else if (elementType === constants.BSON_DATA_REGEXP && bsonRegExp === false) {
      // Get the start search index
      i = index;
      // Locate the end of the c string
      while (buffer[i] !== 0x00 && i < buffer.length) {
...
***REMOVED*** else if (elementType === constants.BSON_DATA_REGEXP && bsonRegExp === true) {
      // Get the start search index
      i = index;
      // Locate the end of the c string
      while (buffer[i] !== 0x00 && i < buffer.length) {
...
***REMOVED*** else if (elementType === constants.BSON_DATA_SYMBOL) {
      const stringSize =
        buffer[index++] |
        (buffer[index++] << 8) |
        (buffer[index++] << 16) |
  ...
  ***REMOVED***
      ...
      index = index + stringSize;
***REMOVED*** else if (elementType === constants.BSON_DATA_TIMESTAMP) {
      const lowBits =
        buffer[index++] |
        (buffer[index++] << 8) |
        (buffer[index++] << 16) |
...
***REMOVED*** else if (elementType === constants.BSON_DATA_MIN_KEY) {
      value = new MinKey();
***REMOVED*** else if (elementType === constants.BSON_DATA_MAX_KEY) {
      value = new MaxKey();
***REMOVED*** else if (elementType === constants.BSON_DATA_CODE) {
      const stringSize =
        buffer[index++] |
        (buffer[index++] << 8) |
        (buffer[index++] << 16) |
  ...
  ***REMOVED***
      ...
      index = index + stringSize;
***REMOVED*** else if (elementType === constants.BSON_DATA_CODE_W_SCOPE) {
      const totalSize =
        buffer[index++] |
        (buffer[index++] << 8) |
        (buffer[index++] << 16) |
  ...
  ***REMOVED***
***REMOVED*** else if (elementType === constants.BSON_DATA_DBPOINTER) {
      // Get the code string size
      const stringSize =
        buffer[index++] |
        (buffer[index++] << 8) |
